...
def format_gearbox(file_name, segment_length, nperseg):
	...
	for i in range(splitted_val.shape[0]):
		...
		for j in range(splitted_val.shape[2]):
			f, t, sxx = spectrogram(splitted_val[i, :, j], fs=frequency, nperseg=nperseg)
			s_X.append(sxx)
			val_X.append(splitted_val[i, :, j])

		y.append(df['label'][0])
		X1.append(np.stack(s_X))
		X2.append(np.stack(val_X))
...
def prepare_gearbox(data_folder, segment_length, nperseg):
	...
	for file in files_domain0:
		print(file)
		s_X, v_X,  _y = format_gearbox(file, segment_length, nperseg)
		s_X_domain0.append(s_X)
		v_X_domain0.append(v_X)
		y_domain0.append(_y)
	
	for file in files_domain1:
		print(file)
		s_X, v_X,  _y = format_gearbox(file, segment_length, nperseg)
		s_X_domain1.append(s_X)
		v_X_domain1.append(v_X)
		y_domain1.append(_y)

	s_X_domain0 = np.concatenate(s_X_domain0)
	s_X_domain0 = (s_X_domain0 - s_X_domain0.mean(axis=(0, -2, -1), keepdims=True)) / (s_X_domain0.std(axis=(0, -2, -1), keepdims=True)+1e-6)
	v_X_domain0 = np.concatenate(v_X_domain0)
	v_X_domain0 = (v_X_domain0 - v_X_domain0.mean(axis=(0, -1), keepdims=True)) / (v_X_domain0.std(axis=(0, -1), keepdims=True)+1e-6) 

	y_domain0 = np.concatenate(y_domain0)
	y_domain0 = y_domain0.reshape(-1, 1)

	s_X_domain1 = np.concatenate(s_X_domain1)
	s_X_domain1 = (s_X_domain1 - s_X_domain1.mean(axis=(0, -2, -1), keepdims=True)) / (s_X_domain1.std(axis=(0, -2, -1), keepdims=True)+1e-6)
	v_X_domain1 = np.concatenate(v_X_domain1)
	v_X_domain1 = (v_X_domain1 - v_X_domain1.mean(axis=(0, -1), keepdims=True)) / (v_X_domain1.std(axis=(0, -1), keepdims=True)+1e-6) 
	...
	data_name = os.path.split(data_folder)[-1]
	np.savez(os.path.join(save_path, data_name + "_"+ "0.npz"), x=v_X_domain0, y=y_domain0)
	np.savez(os.path.join(save_path, data_name + "_"+ "1.npz"), x=v_X_domain1, y=y_domain1)
	np.savez(os.path.join(save_path, data_name + "_"+ "0_spectrogram.npz"), x=s_X_domain0, y=y_domain0)
	np.savez(os.path.join(save_path, data_name + "_"+ "1_spectrogram.npz"), x=s_X_domain1, y=y_domain1)
