...
class ASTModel(nn.Module):
    ...
    def __init__(self, label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024, projection_dim=2048, imagenet_pretrain=True, audioset_pretrain=False, model_size='base384', verbose=True, C_in=1):
        ...
        self.mlp_head = nn.Sequential(nn.Linear(self.projection_dim, label_dim+1), nn.Softmax(dim=1))
    ...
    def forward(self, x, alpha, is_source=True):
        ...
        prob_p_dis = x_out[:, -1].unsqueeze(1)
        prob_p_class = x_out[:, :-1]
        prob_p_class = prob_p_class / (1-prob_p_dis+1e-8)

        return prob_p_class, prob_p_dis, x_out
