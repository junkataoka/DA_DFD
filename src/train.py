...
def train_batch(model, src_train_batch, tar_train_batch, 
    ...
    alpha2 = torch.tensor((np.exp(10 * p) - 1) / (np.exp(10) - 1)).cuda()
    ...
    enc_nll_src_class = F.log_softmax(src_class, dim=1)
    enc_nll_src_dis = F.log_softmax(src_dis, dim=1)
    enc_nll_tar_dis = F.log_softmax(tar_dis, dim=1)
    ...
    if args.use_domain_adv:
        loss += (enc_loss_src_dis + enc_loss_tar_dis) / 2

    if args.use_tar_entropy:
    ...
    cls_nll_src_class = F.log_softmax(src_class, dim=1)
    cls_nll_src_dis = F.log_softmax(src_dis, dim=1)
    cls_nll_tar_dis = F.log_softmax(tar_dis, dim=1)
    ...
    if args.use_domain_adv:
        loss += (cls_loss_src_dis + cls_loss_tar_dis) / 2
